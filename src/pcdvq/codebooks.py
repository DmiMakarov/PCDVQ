...
class Codebook:
    def __init__(self, direction_bits: int = 16, magnitude_bits: int = 2):
        ...
        self.magnitudes = torch.randn(2**magnitude_bits)
        self.directions = torch.randn(2**direction_bits, 7)
    ...
    def construct_direction_codebook(self) -> list[torch.Tensor]:
        ...
        self.directions = self._get_direction_from_vectors(torch.cat([d8_signs, d8_signs + 0.25], dim=0))
    ...
    def construct_magnitude_codebook(
        self, k: int = 8, tau: float = 0.99, tol: float = 1e-8, max_iters: int = 100
    ) -> torch.Tensor:
        ...
        for _ in range(max_iters):
            ...
            for i in range(num_centers):
                num = scipy.special.gammainc((k + 1) / 2.0, np.float32((u[i + 1] ** 2) / 2.0)) - scipy.special.gammainc(
                    (k + 1) / 2.0, np.float32((u[i] ** 2) / 2.0)
                )
                den = scipy.special.gammainc(k / 2.0, np.float32((u[i + 1] ** 2) / 2.0)) - scipy.special.gammainc(
                    k / 2.0, np.float32((u[i] ** 2) / 2.0)
                )
        ...
        self.magnitudes = codebook

    def save_codebooks(self, path: str = ""):
        """Save the codebooks to a file."""
        torch.save(self.directions, path + "codebook_direction.pt")
        torch.save(self.magnitudes, path + "codebook_magnitude.pt")

    def load_codebooks(self, path: str = ""):
        """Load the codebooks from a file."""
        self.directions = torch.load(path + "codebook_direction.pt")
        self.magnitudes = torch.load(path + "codebook_magnitude.pt")
